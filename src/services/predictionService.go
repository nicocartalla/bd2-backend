...
func (s *PredictionService) GetPredictionsByUser(userID int) ([]models.Prediction, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&prediction.PredictionID, &prediction.UserID, &prediction.MatchID, &prediction.GoalsLocal, &prediction.GoalsVisitor); err != nil {
...
func (s *PredictionService) InsertPrediction(prediction models.Prediction) (int64, error) {
	...
	result, err := database.InsertDBParams(query, prediction.GoalsLocal, prediction.GoalsVisitor, prediction.UserID, prediction.MatchID, prediction.GroupID)
	if err != nil {
